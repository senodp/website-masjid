					...
					<div class="row">
						<div class="col-md-8">
							<div class="form-group">
								<label for="cover">Background Image Header</label>

								<?php if($this->uri->segment(2) == 'article' || $this->uri->segment(2) == 'articles' || $this->uri->segment(2) == 'insights'){ ?>
									<?=html_upload_img($row['cover'],'cover','listpages','466x326');?>
								<?php }else{ ?>
									<?=html_upload_img($row['background_image'],'background_image','listpages','1920x1080');?>	
								<?php } ?>
						...
						<div class="col-md-4">
							<div class="form-group">
								<label for="cover">Icon Image</label>

								<?php if($this->uri->segment(2) == 'article' || $this->uri->segment(2) == 'articles' || $this->uri->segment(2) == 'insights'){ ?>
									<?=html_upload_img($row['cover'],'cover','listpages','466x326');?>
								<?php }else{ ?>
									<?=html_upload_img($row['icon_image'],'icon_image','listpages','300x300');?>	
								<?php } ?>
							...
							<div class="tab-content">
								<div class="tab-pane active" id="mt-eng">
									...
									<div class="form-group">
										...
										<input type="text" class="form-control <?=error_class('title_content')?>" name="title_content" value="<?=set_row_value('title_content',$row)?>" />
										<?=error_block('title_content')?>
									</div>
									<div class="form-group">
					...
					<div class="row">
						...
						<div class="col-md-4">
							<div class="form-group">
								...
								<?php }else{ ?>
									<br>
									<small>recommended image size is 800x600 pixels</small>
									<img width="250" src="<?=img_url($row['cover'], 'listpages')?>">
									<?=html_upload($row['cover'],'cover','listpages');?>	
								<?php } ?>

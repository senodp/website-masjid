...
var initSortable = function(){
	$('.is-sortable').each(function(){
		let sort_parent = this;
		let sort_url = $(this).data('post-url');

		new Sortable(sort_parent, {
			animation: 150,
			ghostClass: 'bg-blue',
			onEnd: function(){
				let sorted = $(sort_parent).children().first().data('id');

				$(sort_parent).children().each(function( index ){
					//console.log($(this));
					if (index > 0){
						sorted = sorted + ',' + $(this).data('id');
					}
				})
				...
				var fD = new FormData();
				fD.append("sorted", sorted);
				fD.append("is_ajax", "1");
				fD.append("is_submit", "1");

				initWaitMe(sort_parent, 'Saving changes');
				
				$.ajax({ url: sort_url, dataType: 'html', data: fD, processData: false, contentType: false, method: 'POST' }).done(function(html){	
					if (html == 'success'){
						window.setTimeout(function(){
							$(sort_parent).waitMe('hide');
						}, 500);
					}
				});
			}
			...
			if (summerEditor.is(':visible')){
				summerOptions = {
					...
					toolbar: [
						...
						['insert', ['link','mediamanager','jTable']],
...
var ajaxCRUD = {
	...
	events: function(modalId){
		...
		$(modalId).find("input[data-role=tagsinput], select[multiple][data-role=tagsinput]").tagsinput();
